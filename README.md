...
| `miaospeed.build_token_env` / `build_tokens` | MiaoSpeed Challenge 签名所需 build token；默认填入 AirportR 预编译二进制公开 token，环境变量优先 |
...
v4 开始接入 [AirportR/miaospeed](https://github.com/AirportR/miaospeed)。
...
```bash
# 本地 macOS arm64
scripts/download_miaospeed.sh --os darwin --arch arm64 --version 4.6.8

# 远端 Linux x86_64 / Docker amd64
scripts/download_miaospeed.sh --os linux --arch amd64 --version 4.6.8

# Linux arm64
scripts/download_miaospeed.sh --os linux --arch arm64 --version 4.6.8
```
...
```bash
scripts/download_miaospeed.sh --os linux --arch amd64 --version 4.6.8 --print-url
GITHUB_PROXY="https://your-github-proxy.example" \
  scripts/download_miaospeed.sh --os linux --arch amd64 --version 4.6.8
DOWNLOAD_CONNECT_TIMEOUT=5 DOWNLOAD_MAX_TIME=30 \
  scripts/download_miaospeed.sh --os linux --arch amd64 --version 4.6.8
...
- 使用外部 MiaoSpeed 时配置 `miaospeed.ws_url`；需要 Go 后端托管时同时设置
  ...
  与 `miaospeed.ws_url` 的 host:port 传给 AirportR 4.6.8 release；
  如果你填写 `miaospeed.args`，后端会按自定义参数启动
- 配置 `MIAOSPEED_TOKEN`；AirportR 预编译二进制公开 build token 已内置到默认配置。
  如果你自行编译 MiaoSpeed，则把 `MIAOSPEED_BUILD_TOKENS` 或 `miaospeed.build_tokens`
  改成对应 `BUILDTOKEN.key`，多个 token 用 `|` 分隔
...
当前适配已切到 AirportR 4.6.x 请求结构：`Configs.ApiVersion` 默认 3，
`Vendor=Clash` 会参与 Challenge 签名，并保留上传测速字段扩展位。下一步继续用
AirportR 4.6.8 预编译二进制验证生产 DNS/解锁脚本输出。

后续接入顺序：

- 使用 AirportR 4.6.8 预编译二进制做生产 DNS/解锁脚本联调
