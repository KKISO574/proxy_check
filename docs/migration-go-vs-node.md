...
6. **高级探测后端趋向 Go 生态**：后续计划接入
   `AirportR/miaospeed` 做 DNS leak、解锁、跑量/带宽测试。Go 后端
...
| 4 | 进行中 | Go-managed MiaoSpeed sidecar；当前已有 Go 请求构造、Challenge 签名、WebSocket client、响应 frame 归一化、sidecar 生命周期、AirportR 4.6.8 release 下载与 `server -token ... -bind ...` 启动方式、`miaospeed_bandwidth`、基于 `TEST_SCRIPT` 的 `miaospeed_dns_leak` / `miaospeed_unlock` prober 和任务级高级探测开关；下一步补 AirportR 4.6.8 真实 sidecar 生产 DNS/解锁脚本联调。 |
...
和可选 `MIAOSPEED_BUILD_TOKENS`。当前主线已改为 AirportR 4.6.x 请求结构：
`Configs.ApiVersion` 默认 3，`Vendor=Clash` 参与 Challenge 签名，默认 build token
使用 AirportR 预编译二进制公开值。下一步用 AirportR 4.6.8 发布二进制验证
`TEST_PING_CONN`、`TEST_SCRIPT`、`Vendor=Clash` 节点 payload 和 `SPEED_*` 带宽矩阵链路。
